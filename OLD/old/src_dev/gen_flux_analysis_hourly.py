...
def genFluxAnalysis(
    ...
    if time_avg_interval / pd.Timedelta(seconds=1) == 0:
    ...
    number_of_intervals = (time_end - time_beg) / time_avg_interval
    ...
    for i in range(number_of_intervals):
        ...
        selected_time_beg = time_beg + time_avg_interval * i
        selected_time_end = selected_time_beg + time_avg_interval

        ds_tmp = new_ds.where(
            ( new_ds.coords["time"] >=  selected_time_beg ) &
            ( new_ds.coords["time"] <   selected_time_end )
        )

        ds_tmp = ds_tmp.mean(dim="time", skipna=True, keep_attrs=True).expand_dims(
            dim = {"time": [selected_time_beg,]},
            axis = 0,
        )
        
        merged_data.append(ds_tmp)
        
     
    new_ds = xr.merge(merged_data)
...
if __name__ == "__main__":
    ...
    ds = genFluxAnalysis(
        ...
        args.time_rng,
        pd.Timedelta(minutes=args.time_avg_interval),
        x_rng,
    )
